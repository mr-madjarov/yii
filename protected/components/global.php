...
/**
 * This is the shortcut to Yii::app()->clientScript->registerCoreScript
 */
 ...
 * @param string $size
 * @param array  $options
 * @return string with the image html
 */
 ...
 * @param integer $limit    the maximum length of the text to be returned. If 0, it means no truncation.
 * @param string  $readMore the label of the "read more" button if $limit is greater than 0.
...
function nh( $text, $limit = 0, $readMore = 'read more' )
{
    if ( $limit && strlen( $text ) > $limit ) {
        ...
        if ( $readMore !== false ) {
            ...
            return nl2br( htmlspecialchars( $ltext, ENT_QUOTES, Yii::app()->charset ) ) . ' ' . l( h( $readMore ), '#',
                array( 'class' => 'read-more', 'onclick' => '$(this).hide().next().show(); return false;' )
            ) . '<span style="display:none;">' . nl2br( htmlspecialchars( $rtext, ENT_QUOTES, Yii::app()->charset )
            ) . '</span>';
        } else
